...
const createtable_1 = __importStar(require("../migration/types/createtable"));
    ...
    for (relationship of constructor.relations) {
        ...
        if (relationship.type === 'manytoone') {
            if (relationEntity &&
                ...
                const relationId = relationEntity.entity.columns.find((f) => f.name === relationEntity.entity.id);
                if (!relationId) {
                    throw Error(`Unknown id`);
                }
                const field = new createtable_1.Field(`${relationship.fieldName}_id`, relationId.getType());
                field.foreign(relationEntity.entity.tableName, relationEntity.entity.id);
            ...
            if (relationEntity && relationEntity.entity.ready) {
                if (relationEntity.entity.initialized) {
                    ...
                    const field = new createtable_1.Field(`${relationEntity.entity.tableName}_id`, relationId.getType());
                    field.foreign(relationEntity.entity.tableName, relationEntity.entity.id);
                    const myId = constructor.columns.find((f) => f.name === constructor.id);
                    if (!myId) {
                        throw Error(`Unknown id`);
                    }
                    const otherfield = new createtable_1.Field(`${constructor.tableName}_id`, myId.getType());
