...
class GlobalPostgreModel extends global_sql_1.default {
    constructor() {
        ...
        this.select = (tableName, distinct, attributes, wheres, sorts, limit, offset = 0) => __awaiter(this, void 0, void 0, function* () {
            const query = `SELECT${distinct ? ' DISTINCT' : ''}${this.computeAttributes(attributes)} FROM \`${tableName}\` AS default_table ${this.computeWhere(wheres, '$', true)}${this.computeSort(sorts)}${limit !== -1 ? ` LIMIT ${limit} OFFSET ${offset}` : ''}`;
